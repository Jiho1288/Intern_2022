...
#과제 1 풀이
import math                                        #math() 모듈 사용/조건충족
a, b, c = map(float, input("input : ").split())    # a, b, c에 input으로 외부값을 받고 이를 split를 이용해 쪼갠 후 이를 실수로 변환하고 이를 map을 이용해 각각 a,b,c에 할당  /a, b, c에 3각형의 길이 저장
s = (a + b + c) / 2                                # 헤론의 공식을 이용 이용해 높이 계산하여 S에 저장
S = math.sqrt(s * (s - a) * (s - b) * (s - c))     # s(s-a)(s-b)(s-c)의 제곱근 계산하여 s에 저장
print(round(S, 3))                                 #결과 프린트

#----------------------------------------------
#과제 2.  아서 왕의 방 안에 존재하는 삼각형 모양 창문의 세 변의 길이가 주어질 때, 
#        해가비추는 영역에 들어갈 수 있는 가장 큰 원탁의 반지름을 구하시오.
